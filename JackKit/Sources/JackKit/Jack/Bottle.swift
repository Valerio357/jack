...
public final class Bottle: ObservableObject, Equatable, Hashable, Identifiable, Comparable, @unchecked Sendable {
    ...
    public static func createGameBottle(name: String, appID: Int) async throws -> Bottle {
        let bottleURL = BottleData.defaultBottleDir.appending(path: "game_\(appID)")
        
        // 1. Create directory
        if !FileManager.default.fileExists(atPath: bottleURL.path(percentEncoded: false)) {
            try FileManager.default.createDirectory(at: bottleURL, withIntermediateDirectories: true)
        }
        
        // 2. Initialise basic structure
        ...
        // 3. Create symlinks to shared Steam
        let programFiles = driveC.appending(path: "Program Files (x86)")
        try FileManager.default.createDirectory(at: programFiles, withIntermediateDirectories: true)
        
        let steamLink = programFiles.appending(path: "Steam")
        let sharedSteam = BottleData.steamDir
        
        // If it exists but is not a symlink, remove it to be safe
        var isDir: ObjCBool = false
        if FileManager.default.fileExists(atPath: steamLink.path(percentEncoded: false), isDirectory: &isDir) {
            if !isDir.boolValue || (try? steamLink.resourceValues(forKeys: [.isSymbolicLinkKey]).isSymbolicLink) != true {
                try FileManager.default.removeItem(at: steamLink)
            }
        }
        
        if !FileManager.default.fileExists(atPath: steamLink.path(percentEncoded: false)) {
            try FileManager.default.createSymbolicLink(at: steamLink, withDestinationURL: sharedSteam)
        }
        ...
        // 4. Important: Initialise the prefix so it's ready for Steam
        // This prevents hangs on first run
