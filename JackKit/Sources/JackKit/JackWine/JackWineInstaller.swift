...
public class JackWineInstaller {
    /// The Jack application folder
    public static let applicationFolder = FileManager.default.urls(
        for: .applicationSupportDirectory, in: .userDomainMask
        )[0].appending(path: Bundle.jackBundleIdentifier)
    ...
    public static func isJackWineInstalled() -> Bool {
    ...
    public static func install(from: URL) {
        do {
            ...
            print("Failed to install JackWine: \(error)")
        }
    }
    ...
    public static func uninstall() {
        do {
            ...
            print("Failed to uninstall JackWine: \(error)")
        }
    }

    public static func shouldUpdateJackWine() async -> (Bool, SemanticVersion) {
        let versionPlistURL = "https://data.getwhisky.app/Wine/JackWineVersion.plist"
                ...
                URLSession(configuration: .ephemeral).dataTask(with: URLRequest(url: remoteUrl)) { data, _, error in
                    do {
                        if error == nil, let data = data {
                            ...
                            let remoteInfo = try decoder.decode(JackWineVersion.self, from: data)
    ...
    public static func whiskyWineVersion() -> SemanticVersion? {
        do {
            let versionPlist = libraryFolder
                .appending(path: "JackWineVersion")
                .appendingPathExtension("plist")

            let decoder = PropertyListDecoder()
            let data = try Data(contentsOf: versionPlist)
            let info = try decoder.decode(JackWineVersion.self, from: data)
            return info.version
        } catch {
            print(error)
            return nil
...
struct JackWineVersion: Codable {
