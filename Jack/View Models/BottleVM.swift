...
final class BottleVM: ObservableObject, @unchecked Sendable {
    ...
    @MainActor
    func getSharedSteamBottle() -> Bottle? {
        return bottles.first(where: { $0.url == BottleData.sharedDir })
    }

    @MainActor
    func createSharedSteamBottle() async throws -> Bottle {
        let sharedURL = BottleData.sharedDir
        if !FileManager.default.fileExists(atPath: sharedURL.path(percentEncoded: false)) {
            try FileManager.default.createDirectory(at: sharedURL, withIntermediateDirectories: true)
        }
        
        let bottle = Bottle(bottleUrl: sharedURL, isAvailable: true)
        bottle.settings.name = "Jack Shared"
        bottle.settings.windowsVersion = .win10
        
        if !bottlesList.paths.contains(sharedURL) {
            bottlesList.paths.append(sharedURL)
        }
        loadBottles()
        return bottle
    }
    ...
    @MainActor
    func createGameBottle(gameName: String, appID: Int) async throws -> Bottle {
        let bottle = try await Bottle.createGameBottle(name: gameName, appID: appID)
        if !bottlesList.paths.contains(bottle.url) {
            bottlesList.paths.append(bottle.url)
        }
        loadBottles()
        return bottle
    }
